...
def execute_commands(hostname, password, username, local_files, remote_dir, commands):
    ...
    # stdin, stdout, stderr = client.exec_command(f'git clone https://github.com/ltrix07/Ebay-Checker.git')
    # print(f'{hostname} ---> Command done.')

    # stdin, stdout, stderr = client.exec_command(f'rm nohup.out')
    # print(f'{hostname} ---> Command done.')
    #
