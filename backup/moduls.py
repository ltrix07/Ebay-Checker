...
from settings import shop_name, col_names, parse_keys_ship_date, parse_keys_price, parse_keys_ship_price, \
    parse_keys_quantity, parse_keys_supplier_name, parse_select_div, headers, parse_catalog_check, \
    main_block, parse_title_h1, what_need_to_parse, server_host, parse_keys_voltage, parse_keys_speed, \
    parse_keys_color, parse_keys_wattage, parse_keys_description, parse_keys_unit_count, parse_keys_item_weight, \
    parse_keys_included_components, parse_keys_part_number, parse_keys_number_of_boxes, parse_keys_power_source, \
    parse_keys_product_dimensions, parse_keys_material_type, strategy
...
class RequestsToEbay:
    ...
    def __add_to_report(self, new_data, previous_price, previous_shipping_price, previous_qty):
        ...
        if int(previous_qty) == 0 and int(new_data["data"]["quantity"]) != int(previous_qty):
            self.report["stock_new"] += 1
        if int(previous_qty) != 0 and int(new_data["data"]["quantity"]) == 0:
    ...
    async def __find_in_page_by_slicing(page, strs=list):
        for str_ in strs:
            try:
                index = page.index(str_['s'])
                index2 = page.index(str_['e'], index + len(str_['s']))
                return page[index + len(str_['s']):index2]
            except ValueError:
                continue
    ...
    async def __parser_page(self, page, row):
        ...
        tasks = {
            ...
            "quantity_supp": self.__find_in_page_by_slicing(page, parse_keys_quantity) if what_need_to_parse[
                "quantity"] else None,
            "ship_date_supp": self.__find_in_page_by_xpath(tree, parse_keys_ship_date) if what_need_to_parse[
        ...
        try:
            for key, task in tasks.items():
                if task is not None:
                    results[key] = await asyncio.create_task()
                else:
                    results[key] = None
        ...
        if 'Varies' in results["main_block_info"][0].text_content() and results["ship_price_supp"] is None:
            ...
            if results["ship_date_supp"]:
                try:
                    if results["ship_date_supp"][0]:
                        results["ship_date_supp"] = await self.__date_to_days(results["ship_date_supp"][0])
                    else:
                        raise Exception(f'Shipping days is None in {url}')
                except ValueError:
    ...
    async def __fetch(self, session, row, proxy_url, proxy_auth):
        sku, url, price, shipping_price, stock, shipping_days, \
            supplier_name = row[self.sku_index], row[self.url_index].split('?')[0].replace(' ', ''), \
            row[self.price_index], row[self.shipping_price_index], row[self.quantity_index], \
            row[self.shipping_days_index], row[self.supplier_index]
        ...
        if url != '' and 'ebay' in url and url != col_names['url']:  # Если ссылка пустая, не пытаемся к ней стучаться
            ...
            try:
                return await self.__get_response(session, row, proxy_url, proxy_auth, sku=sku, variation=variation)
            except aiohttp.client_exceptions.InvalidURL:  # В случае ошибки плохой ссылки выводим ссылку с ошибкой
                if url != col_names['url']:
            ...
            error_message = f'{sku} - ссылка ведёт не на сайт Ebay. Нужно проверить.'
            await self.server_connect.send_message(error_message)
    ...
    async def __get_response(self, session, row, proxy_url, proxy_auth, sku, variation):
        ...
        try:
            ...
            return self.__error_output('time out error', row[1], row[0], row[-1])
        except aiohttp.client_exceptions.ClientHttpProxyError:
            return self.__error_output('proxy error', row[1], row[0], row[-1])
...
class RequestToServer:
    ...
    async def post_report(shop, report, average_time_by_link, time_of_code_processing, proxies):
        data = {
            ...
            'bad_info_perc': round(report['proxy_errors'] / report['all_processed'], 2),
    ...
    @staticmethod
    async def send_file(message_type, shop, file_path):
        async with websockets.connect('ws://' + server_host) as websocket:
...
class FilesWorker:
    ...
    def create_file_for_amazon(data, indices):
        ...
        for row in data:
            input_to_file.append([row[indices["sku"]], row[indices["price"]], row[indices["quantity"]],
    ...
    def append_error_to_file_with_errors(file_path, file_name, sku, error):
        with open(f'./{file_path}/{file_name}', 'a', newline='') as csvfile:
            writer = csv.writer(csvfile)
            ...
            for row in csv_reader:
                ...
                data.append({
                    ...
                    'data': {
                        'sku': sku,
                        'price': float(price),
                        'ship_price': float(shipping_price),
